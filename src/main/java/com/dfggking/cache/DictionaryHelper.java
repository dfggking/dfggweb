...
//			dictionaryList = dictService.listDict();
	 ...
	 * @since 1.0
	 */
	public List<DicDO> getSysDictListByCode(String code) {
		DicDO v = getSysDictByCode(code);
		if(v != null){
			List<DicDO> list = new ArrayList<DicDO>();
			for (DicDO dic : dictionaryList) {
	 ...
	 * @since
	 */
	public List<DicDO> getSysDictByFid(String fid) {
		List<DicDO> list = new ArrayList<DicDO>();
	 ...
	 * @since
	 */
	public DicDO getRootSysDictByName(String name) {
	 ...
	 * @since
	 */
	public DicDO getSysDictByCode(String code) {
		for (DicDO vo : dictionaryList) {
	...
	public String getSysDictNameByCode(String code){
		DicDO vo = getSysDictByCode(code);
		if(vo != null){
			return vo.getDicName();
		}
		return "";
	 ...
	 * @since
	 */
	public void getDictIds(String fid, List<DicDO> result) {
		if (result == null) {
			result = new ArrayList<DicDO>();
		}
		List<DicDO> list = getSysDictByFid(fid); 
		result.addAll(list);
		for (DicDO vo : list){
	...
	public String getStringOptions(String parentNodeName, String nodeName, String selectId, String selected, String definedAttr){
		...
		DicDO vo = Objects.isNull(parentNodeName) ? getRootSysDictByName(nodeName) : getSysDictByNames(parentNodeName, nodeName);
		if(vo != null){
			String id = vo.getId();
			List<DicDO> voList=getSysDictByFid(Objects.isNull(id) ? vo.getParentId() : id);
			for(int i=0;i<voList.size();i++){
				DicDO dict = voList.get(i);
	...
	public String getStringOptions(String parentNodeName,String nodeName,String selectId,String selected,String definedAttr,String indexOptionStr){
		...
		for (int j=0;j<nodeNames.length;j++) {
			DicDO t = Objects.isNull(parentNodeName) ? getRootSysDictByName(nodeNames[j]) : getSysDictByNames(parentNodeName,nodeNames[j]);
			if(t != null){
				String id = t.getId();
				List<DicDO> table=getSysDictByFid(Objects.isNull(id) ? t.getParentId() : id);
				for(int i=0;i<table.size();i++){
					DicDO dict = table.get(i);
					String nodeId = dict.getId();
					String nodeText = dict.getDicName();
					if(nodeId.equals(selected)) 
						sb.append("<option value='"+nodeId+"' selected>"+nodeText+"</option>\n");
	...
	private List<DicDO> getDescendantDicByCode(String currentCode,int level,int [] levels,boolean containCur,List<DicDO> childs){
		...
		if(childs==null){
			childs=new ArrayList<DicDO>();
			DicDO currenDictNode=getSysDictByCode(currentCode);
			if(containCur&&currenDictNode!=null){
				childs.add(currenDictNode);
			}
		}
		  List<DicDO> nextLevelDics = getSysDictListByCode(currentCode);
			if(nextLevelDics!=null){
				...
				if(level>1){
					for(DicDO DictNode : nextLevelDics){
