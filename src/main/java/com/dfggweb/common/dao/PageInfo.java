...
import com.dfggweb.util.prop.PropDefault;
import com.dfggweb.util.prop.PropKey;
...
public class PageInfo {
    ...
    /**
     * 计算分页信息，计算规则需要符合，每页条数不得小于等于0；总页数不得小于等于0，总页数为总行数与每页条数的商如果有余数则加1，当前页数不得大于总页数 ，当前页数不得小于1，起始条数为当前页数减1再乘以每页条数。分页的计算无法保证实时性，每次发生变化时建议重新计算。
     */
    public void handle() {
